...
const test_reports_dir = `test_reports/results_${Math.ceil(Math.random() * 100)}/`
process.env.TEST_REPORTS_DIR = test_reports_dir;
const os_username = os.userInfo().username;
const git_branch = execSync('git rev-parse --abbrev-ref HEAD').toString().trim();
const report_card_path = `${test_reports_dir}/report_card.json` // Read the report_card.json file to upload
...
upload_directory(AWS_BUCKET_NAME, `${test_reports_dir}`);